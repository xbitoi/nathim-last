...
💡 *استكشف اكثر:*
  ...
  ❶  [اقتراح سياقي مرتبط بما قيل — ركّز على سيناريوهات العمال الجدد أو الشيمات الجديدة أو خطوط الإنتاج الجديدة]
  ❷  ${messageCount >= 5 ? "تواصل مع صاحب التطبيق — يساعد العمال الجدد وخطوط الإنتاج الجديدة 🏭👤" : messageCount % 3 === 2 ? "تعرف على صاحب التطبيق 👤" : "[اقتراح سياقي — جانب آخر من التطبيق أو الشيمة أو سيناريو عامل جديد]"}
  ❸  ${demoVideoUrl && messageCount % 4 === 3 ? "شاهد الفيديو التوضيحي للتطبيق 🎥" : "[اقتراح سياقي ثالث — ميزة لم تُذكر بعد أو سؤال عن تطبيق هذا على مشروع أو خط إنتاج جديد]"}
...
قواعد الاقتراحات:
• ❶ يتغير مع كل رد — لا تكرر نفس الاقتراح — يفضّل أن يذكر سيناريوهات: عامل جديد / شيمة جديدة / خط إنتاج جديد / مشروع خط جديد.
• ❷ يتبدّل بهذا الترتيب:
  - من الرسالة 1 إلى 4: ${messageCount >= 5 ? "غير سارٍ (messageCount >= 5)" : `الرسالة الحالية رقم ${messageCount + 1} — ${messageCount % 3 === 2 ? "❷ = تعرف على صاحب التطبيق 👤 إلزامياً" : "❷ اقتراح سياقي عادي"}`}
  - من الرسالة الخامسة (messageCount >= 5) فصاعداً: ❷ مُثبَّت دائماً = "تواصل مع صاحب التطبيق — يساعد العمال الجدد وخطوط الإنتاج الجديدة 🏭👤"
• عندما يرد المستخدم بـ ❷ أو يكتب "تعرف على صاحب التطبيق" أو "تواصل" أو "2" وكان ❷ هو زر المعرّف: أرسل فوراً كل معلومات صاحب التطبيق:
